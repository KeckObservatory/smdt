...
parser = argparse.ArgumentParser(description="SMDT Mask Design")
parser.add_argument(
    "-i","--instrument",
    type=str.lower,
    choices=["lris", "deimos"],
...
# @app.route('/readparams')         ######### This route doesn't do anything as params_schema in utils now does stuff!
def readparams():
    params = {}
        ...
        for line in f:
            try:
                if len(line.split(',')) == 4:
                    ...
                    (k, v) = sep[0].split(' = ')
                    params[k] = (stripquote(v), stripquote(sep[1]),
    ...
    if instrument == "lris":
        ...
        tel_params = ("Keck II", "", "Telescope", "Keck I or Keck II")
        inst_params = ("DEIMOS", "", "Instrument", "LRIS or DEIMOS")
    params["Telescope"] = tel_params
    params["Instrument"] = inst_params
    return params
