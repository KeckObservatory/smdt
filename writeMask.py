...
BluSlits,C,slitX1,BluSlits,millX1
BluSlits,C,slitY1,BluSlits,millY1
BluSlits,C,slitX2,BluSlits,millX2
BluSlits,C,slitY2,BluSlits,millY2
BluSlits,C,slitX3,BluSlits,millX3
BluSlits,C,slitY3,BluSlits,millY3
BluSlits,C,slitX4,BluSlits,millX4
BluSlits,C,slitY4,BluSlits,millY4
...
#        obsTime = sexg2Float(ts.strftime("%H:%M:%S")) * 15  # to degree
    ...
    def genBluSlits(self):
        ...
        if nSlits > 0:
            ...
            cols.append(pf.Column(name="slitX1", format="F9.3",
                        null="0.000", unit="mm", array=selected.millX1))
            cols.append(pf.Column(name="slitY1", format="F9.3",
                        null="0.000", unit="mm", array=selected.millY1))
            cols.append(pf.Column(name="slitX2", format="F9.3",
                        null="0.000", unit="mm", array=selected.millX2))
            cols.append(pf.Column(name="slitY2", format="F9.3",
                        null="0.000", unit="mm", array=selected.millY2))
            cols.append(pf.Column(name="slitX3", format="F9.3",
                        null="0.000", unit="mm", array=selected.millX3))
            cols.append(pf.Column(name="slitY3", format="F9.3",
                        null="0.000", unit="mm", array=selected.millY3))
            cols.append(pf.Column(name="slitX4", format="F9.3",
                        null="0.000", unit="mm", array=selected.millX4))
            cols.append(pf.Column(name="slitY4", format="F9.3",
                        null="0.000", unit="mm", array=selected.millY4))
    ...
    def writeOut(self, fileTarget):
        """
        Writes output catalog to a file or file-like object
        """
        params = self.params
        tel = self.tel
        tlist = self.slits
        ...
        guides = tlist[(tlist.selected == 1) & (tlist.pcode == -1)]
        gsMajAxPA = np.degrees(guides.slitLPA)
 
        nonselected = tlist[tlist.selected == 0]
        ...
        # Compose all content into one big string first
        content = ""
        content += "# Mask name, center:\n"
        content += f"{params.maskid[0]}             {toSexagecimal(tel.newcenterRADeg/15., secFmt='{:08.5f}')}    {toSexagecimal(tel.newcenterDECDeg, secFmt='{:08.5f}')}  2000.0 PA= {params.pa0[0]} ##\n"
        content += "#\n"
        content += "#  Guider center:\n"
        content += "#\n"
...
class MaskDesignInputFitsFile:
    ...
    def getObjOnSlit(self, slits=None):
        ...
        x0 = (slits.millX1 + slits.millX4) / 2
        y0 = (slits.millY1 + slits.millY4) / 2
        x1 = (slits.millX3 + slits.millX2) / 2
        y1 = (slits.millY3 + slits.millY2) / 2
